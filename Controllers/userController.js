...
const userRegistration = async (req, res) => {
  try {
    ...
    let data = req.body; 
    letCheckDuplicateEmails = await userModel.letCheckDuplicateEmails(req.body.email);
    if (letCheckDuplicateEmails.length > 0) {
      return res
        .status(404)
        .send({
          status: false,
          msg: "Email Already exists please try with new email or Login",
        });
    } else {
let hash = cryptojs.SHA256(req.body.password).toString(cryptojs.enc.Hex)

      let registration = await userModel.registration(data ,hash);
      if (registration) {
    ...
    if (req.body.content !== "") {
      if (checkUserById.length > 0) {
        ...
        if (chatRegister) {
          return res
            .status(201)
            .send({ status: true, data: "Successfully", data: data });
        } else {
          return res
            .status(400)
            .send({ status: false, data: " Something Went Wrong" });
        }
      } else {
        return res
          .status(404)
          .send({ status: false, msg: "no data found by this id" });
      }
    } else {
      return res
        .status(200)
        .send({ status: false, msg: "enter a valid message" });
    }
  } catch (err) {
    return res.status(500).send({ status: false, error: err.message });
  }
...
const getUser = async function (req, res) {
  try {
    ...
    if (getAllDetails) {
      return res
        .status(200)
        .send({ status: true, msg: "Successfull", data: getAllDetails });
    } else {
      return res
        .status(400)
        .send({ status: false, data: "Something went Wrong " });
...
const getUserMsg = async function (req, res) {
  try {
    ...
    if (getDetailsById.length > 0) {
      return res
        .status(200)
        .send({ status: true, msg: "successfull", data: getDetailsById });
    } else {
      return res
        .status(404)
        .send({ status: false, msg: "no data found by this id" });
...
const fetchMsg = async function (req, res) {
  try {
    ...
    if (fetchMsgById.length > 0) {
      return res
        .status(200)
        .send({ status: true, msg: "successfull", data: fetchMsgById });
    } else {
      return res
        .status(404)
        .send({ status: false, msg: "no data found by this id" });
...
const getIdByName = async function (req, res) {
  try {
    ...
    if (fetchId) {
      return res
        .status(200)
        .send({ status: true, msg: "Successfull", data: fetchId });
    } else {
      return res
        .status(400)
